...
%type  <ptr> ext_def_list var_list array_assign value_list constant jump
%token <int_value> INT 
...
%token LP RP  LC RC LB RB PLUS MINUS STAR DIV ASSIGNOP AND OR NOT DPLUS_L DPLUS_R DMINUS_L DMINUS_R
%token COMMA SEMI  ARRAY_CALL
%token RETURN IF IF_ELSE WHILE
...
var
    : ID LB INT RB {$$=mknode(0,ARRAY_ID,yylineno);strcpy($$->type_id, $1); $$->int_value=$3;}
    | ID ASSIGNOP exp  {$$=mknode(1,VAR_INIT,yylineno, $3);strcpy($$->type_id, $1);}
    | ID   {$$=mknode(0,VAR_ID,yylineno);strcpy($$->type_id, $1);}
    | ID LB INT RB ASSIGNOP LB value_list RB {$$=mknode(1,ARRAY_INIT,yylineno, $7);strcpy($$->type_id, $1); $$->int_value=$3;}
    ;
...
constant
    : INT{$$=mknode(0,INT,yylineno);$$->int_value=$1;$$->type=INT;}
    | FLOAT {$$=mknode(0,FLOAT,yylineno);$$->float_value=$1;$$->type=FLOAT;}
    | CHAR  {$$=mknode(0,CHAR,yylineno);$$->char_value=$1;$$->type=CHAR;}
    ;
...
function_declaration
	: ID LP params_list RP {$$=mknode(1,FUN_ID,yylineno, $3);strcpy($$->type_id, $1);}
    | ID LP RP {$$=mknode(0,FUN_ID,yylineno);strcpy($$->type_id, $1);}
	;
...
statement
    ...
    | IF LP exp RP statement %prec LOWER_THEN_ELSE   {$$=mknode(2,IF,yylineno,$3,$5);}
    | IF LP exp RP statement ELSE statement   {$$=mknode(3,IF_ELSE,yylineno,$3,$5,$7);}
    | WHILE LP exp RP statement {$$=mknode(2,WHILE,yylineno,$3,$5);}
    | jump SEMI{$$=$1;}
	;
